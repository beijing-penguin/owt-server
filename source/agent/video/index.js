...
function VMixer(rpcClient, clusterIP) {
    ...
    var addInput = function (stream_id, codec, options, avatar, on_ok, on_error) {
        ...
        if (engine) {
			...
			if (!options.drawtext && arr_name[inputId]){
				...
				options.drawtext = "fontfile=/usr/share/fonts/owt-fonts/FreeSerif.ttf:fontcolor=0xFF4500:fontsize=50:text='abc':x=w-tw:y=h-th"
			}
            if (inputId >= 0) {
    ...
    var addOutput = function (codec, resolution, framerate, bitrate, keyFrameInterval, on_ok, on_error) {
        engine.drawText("fontfile=/usr/share/fonts/owt-fonts/MSYHBD.TTC:fontcolor=0xFF4500:fontsize=50:text='时信会议':x=0:y=0");
